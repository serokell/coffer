...
import Coffer.Path (Path, mkPath, mkPathSegment, unEntryPath, unPathSegment, entryPathName, EntryPath)
...
setFieldCmd (SetFieldOptions entryPath fieldName fieldContentsMb visibilityMb) = do
  readSecret entryPath Nothing >>= \case
...
findCmd (FindOptions pathMb textMb sortMb filters filterFields) = do
  let
    ...
    filterByPath e =
      case textMb of
        Nothing -> True
        Just text -> any (T.isInfixOf text) (e ^.. E.path . unEntryPath . each . to unPathSegment)

    filterByTag :: Entry -> Bool
    filterByTag e =
      case textMb of
        Nothing -> True
        Just text -> any (T.isInfixOf text) (e ^.. E.tags . each . to E.getEntryTag)
...
renameCmd :: (Member BackendEffect r, Member (Embed IO) r) => RenameOptions -> Sem r RenameResult
...
copyCmd (CopyOptions oldPath newPath force) = do
  ...
  -- Validation checks.
  checkDestinatationIsNotDirectory operations
  when (not force) $ checkDestinatationIsNotEntry operations

  -- Write new entries.
  let newEntries = coNew <$> operations
  forM_ newEntries writeSecret
  pure $ CPRSuccess $ operations <&> \op -> (coOld op ^. E.path, coNew op ^. E.path)
  ...
  where
    ...
    checkDestinatationIsNotDirectory :: [CopyOperation] -> Sem r ()
    checkDestinatationIsNotDirectory operations = do
      invalidOperations <- flip filterM operations \(CopyOperation _ new) -> do
        pathIsDirectory (new ^. E.path)
      when (not (null invalidOperations)) do
        invalidOperations
          <&> (\(CopyOperation old new) -> (old ^. E.path, new ^. E.path) )
          & throw . CPRDestinationIsDirectory

    -- | Checks that the paths we're trying to copy entries to aren't already occupied
    -- by an existing entry.
    checkDestinatationIsNotEntry :: [CopyOperation] -> Sem r ()
    checkDestinatationIsNotEntry operations = do
      invalidOperations <- flip filterM operations \(CopyOperation _ new) -> do
        pathIsEntry (new ^. E.path)
      when (not (null invalidOperations)) do
        invalidOperations
          <&> (\(CopyOperation old new) -> (old ^. E.path, new ^. E.path) )
          & throw . CPREntryAlreadyExists
...
tagCmd (TagOptions entryPath tag delete) = do
  ...
  where
    ...
    updateTags :: [EntryTag] -> Sem r [EntryTag]
    updateTags tags = do
      if delete then do
        when (tag `notElem` tags) do
          throw $ TRTagNotFound tag
        pure $ List.delete tag tags
      else do
        when (tag `elem` tags) do
  ...
  where
    ...
    tryGetEntry path =
      case Path.pathAsEntryPath path of
        Left _ -> pure Nothing
        Right entryPath -> readSecret entryPath Nothing
          ...
          forM_ secrets \secret -> do
            ...
            case (mkPathSegment secret, mkPath secret) of
              (Right entryName, _) -> do
                entry <- lift $ readSecret (Path.appendEntryName rootPath entryName) Nothing
                case entry of
