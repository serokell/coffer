...
passFieldPrism = prism' to from
  where
    to :: (E.FieldKey, E.Field) -> PassField
...
instance E.EntryConvertible PassEntry where
  entry = prism' to from
    where
      to :: E.Entry -> PassEntry
      to entry =
        PassEntry
        { peMasterFieldKey = entry ^. E.masterField <&> E.getFieldKey
        , peMasterFieldValue =
            (entry ^. E.masterField)
            >>= \fieldKey -> entry ^. E.fields . at fieldKey
  ...
  entry = prism' to from
    where
      ...
      to entry =
        ...
        in PassKv masterValue . catMaybes $
      ...
      from (PassKv masterValue passKv) = do
        let hs = HS.fromList passKv
            ...
            masterKey = masterValue >>= \val -> fhs ^. at val
